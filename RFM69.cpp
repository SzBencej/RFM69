...
void RFM69::select() {
 ...
 // _spi->beginTransaction(_settings);
  ...
  _spi->setDataMode(0); // SPI_MODE0
  _spi->setBitOrder(1); //msbfirst
  #if defined(__STM32F1__)
    _spi->setClockDivider(SPI_CLOCK_DIV8);
  #elif defined(__arm__)
    _spi->setClockDivider(1); // SPI_CLOCK_DIV16
  #else
...
#define SerialPrint(x) printf("%s", x)
...
void RFM69::listenModeSleep(uint16_t millisInterval) 
{
  ...
  wiringPiISR(_interruptNum, INT_EDGE_RISING, delayIrq);
...
void RFM69::listenModeStart(void)
{
  ...
  //detachInterrupt(_interruptNum);
  wiringPiISR(_interruptNum, INT_EDGE_RISING, listenModeIrq);
  setMode(RF69_MODE_STANDBY);
